...
(function () {
  ...
  window.renderPerfis = function renderPerfis() {
    ...
    const perfis = Array.isArray(window.S.perfis) ? window.S.perfis : [];
    $('perm-user-list').innerHTML = window.S.users.map((u) => `<div class="user-row"><div class="user-info"><div class="user-name">${esc(u.name)}</div><div class="user-login">${esc(u.login)}</div></div><select aria-label="Perfil de ${esc(u.name)}" onchange="vincularPerfilPermissao('${esc(u.id)}',this.value)"><option value="">Sem perfil</option>${perfis.map((p) => `<option value="${esc(p.id)}"${String(p.id) === String(u.perfil_id) ? ' selected' : ''}>${esc(p.nome)}</option>`).join('')}</select></div>`).join('') || '<div class="empty">Nenhum usuário cadastrado.</div>';
  };
